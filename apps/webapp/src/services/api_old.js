...
export const api = {
  ...
  getProductById: async (id) => {
    ...
    return {
      ...
      reviewsCount: data.reviews_count || 127,
      variants: data.attributes?.variants || [{ type: 'size', options: ['1.5kg', '3kg', '7kg', '12kg'] }]
    };
  },
  ...
  addQuestion: async (productId, content, userId) => {
    ...
    const { data, error } = await supabase
      .from('product_questions')
      .insert({ product_id: productId, user_id: userId, question: content })
      .select()
      .single();
